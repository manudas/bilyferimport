...
*
*  @author    Manuel José Pulgar Anguita for Spindok SL
*  @copyright Spindok
*  @version   1.0
...
class spindok extends Module
{
	public function __construct()
	{
		...
		$this->displayName = $this->l('Spindok tracking code for Prestashop');
		$this->description = $this->l('Place the spindok tracking code into your store easily!');

		$this->confirmUninstall = $this->l('Are you sure you want to uninstall?');

		//store selection
		$this->selected_store_id = (!$this->emptyTest( Tools::getValue('spindok_shop_select') ) )
								? (int)Tools::getValue('spindok_shop_select')
	...
	{
		$merchant_querry = spindokModel::getContent( $this->selected_store_id );
		...
		if (empty($merchant_id)) {
			...
			if (!empty($products)){
				...
				for ($index = 0; $index < count($products); $index++){
					...
					// Option A:
					/*
					$product_obj = new Product($product['product_id'], false);
					foreach ($product_obj -> name as $name) {
						if (!empty($name)){
							$product_name = $name;
						}
					}
					*/
					...
					$product_name = $product_obj -> name[$default_language];
			...
			return "<script type='text/javascript'> JSON_AFFILIRED_DEBUGIN_VAR = '".base64_encode($theme_string)."' </script>". $theme_string;
		}
	}
		...
		if (Tools::isSubmit('submit'.$this->name))
		{
			...
			foreach ($ps_shop_list as $shop) {
				...
				if (empty($current_merchant_id)) {
					...
					$spindok_merchant_collection -> where ('id_store' , '=', /*"'".*/$shop['id_shop']/*."'"*/);
					$hydrated_collection = $spindok_merchant_collection -> getAll();
					foreach ($hydrated_collection as $spindok_merchant) {
						$spindok_merchant -> delete();
					}
				}
				else {
					spindokModel::setContent( $current_merchant_id, $shop['id_shop'] );
				}
			}
	...
	public function displayForm()
	{
		...
		$fields_form[]['form'] = array(
				...
				'input' => array(
					array(
						'type' => 'text',
						'name' => 'merchant_id',
						'label' => $this->l("Your merchant ID in Spindok"),
					),
				),
				'submit' => array(
					'title' => $this->l('Save'),
		...
		$selectedShop = intval(Tools::getValue('spindok_shop_select'));

		$selected_spindok_obj = spindokModel::getContent( $this->selected_store_id );

		$content_query = $selectedShop == 0 ? $this -> getMerchantIDMultiStore() : $selected_spindok_obj;
		...
		foreach ($ps_shop_list as $shop) {
			...
			$current_spindok_obj = spindokModel::getContent( $shop['id_shop'] );

			$helper->fields_value['merchant_id_'.$shop['id_shop']] = $current_spindok_obj['merchant_id'];
		}
	...
	private function getMerchantIDMultiStore()
	{
		...
		$number_of_spindok_trackings = count($inflated_collection);
		if ($number_of_shops != $number_of_spindok_trackings) {
			$merchant_value = null;
		}
		else {
			...
			if (!empty($inflated_collection)) {
				foreach ($inflated_collection as $spindok_merchant) {
					if ($firstShop == true) {
						$firstShop = false;
						$merchant_value['merchant_id'] = $spindok_merchant -> merchant_id;
						// die("<pre>".var_export($spindok_merchant, true));
					}
					else {
						if ($value_changed == false) {
							$new_value = $spindok_merchant;
...
class spindokModel extends ObjectModel
{
	public $id_store;
	public $merchant_id;
	...
	public static function createTables()
	{
		...
		return (spindokModel::createContentTable());
	}
